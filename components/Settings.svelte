<script>
	...
	import HealthMonitoring from './Settings/HealthMonitoring.svelte';
...
<button
  class="px-3 py-2 min-w-fit rounded-lg flex items-center transition {selectedTab ===
    'healthMonitoring'
      ? 'bg-gray-100 dark:bg-gray-800'
      : 'hover:bg-gray-50 dark:hover:bg-gray-850'}"
  on:click={() => {
    selectedTab = 'healthMonitoring';
  }}
>
  <div class="self-center mr-2">
    <svg
  ...
  <div class="self-center">{$i18n.t('Health Monitoring')}</div>
</button>
	...
	<div class="flex-1 mt-3 lg:mt-0 overflow-y-scroll">
		...
		{:else if selectedTab === 'documents'}
			...
			/>
		{:else if selectedTab === 'healthMonitoring'}
		<HealthMonitoring
			saveHandler={async () => {
			toast.success($i18n.t('Settings saved successfully!'));
